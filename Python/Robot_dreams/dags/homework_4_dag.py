...
from get_config import get_dshop_tables, get_payload_dates
...
def group_payload(used_date):
    return PythonOperator(
        task_id=f"get_RD_data_{used_date}",
        dag=RD_payload_dag,
...
def postgres_dump(table_name):
    return PythonOperator(
        task_id=f"Copy_postgres_{table_name}",
        dag=RD_payload_dag,
...
for table in get_dshop_tables():
    dummy1 >> postgres_dump(table) >> dummy2
