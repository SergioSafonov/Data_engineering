...
def read_jdbc():
    ...
    with psycopg2.connect(**pg_creds) as pg_connection:
        ...
        cursor.execute('SELECT name FROM test WHERE id < %s', ('3') )
        for row in cursor:
            print(row)
    ...
    with psycopg2.connect(**pg_creds) as pg_connection:
        ...
        with open(file=os.path.join('.', 'data', 'sample.csv'), mode='w') as csv_file:
    ...
    with open(file=os.path.join('.', 'data', 'sample2.csv'), mode='w') as csv_file:
        cursor.copy_expert('COPY ({0}) TO STDOUT WITH HEADER CSV'.format(query), csv_file)

    with psycopg2.connect(**pg_creds) as pg_connection:
        cursor = pg_connection.cursor()
        # copy query from table test to sample.csv
        with open(file=os.path.join('.', 'data', 'sample3.csv'), mode='w') as csv_file:
...
def write_jdbc():
    with psycopg2.connect(**pg_creds) as pg_connection:
        ...
        with open(file=os.path.join('.', 'data', 'sample.csv'), mode='r') as csv_file:
